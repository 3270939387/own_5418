...
class PandaObstacleEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        # frame_skip: number of physics steps per environment step(gym step = how many mujoco steps)
        self.frame_skip = frame_skip
        self.max_episode_steps = max_episode_steps
        self.render_width = render_width
        self.render_height = render_height
        self.goal_reach_threshold = goal_reach_threshold
    # proximity penalty: scale and threshold (if within threshold, penalty applied proportional to closeness)
        self.proximity_penalty_scale = 1.0
        self.proximity_threshold = 0.05

        self.model = mujoco.MjModel.from_xml_path(str(self.model_path))
        self.data = mujoco.MjData(self.model)
        # dt: real time between two environment steps
    ...
    def _min_ee_obstacle_distance(self) -> float:
        ee_pos = self.data.xpos[self.hand_body_id]
        obstacle_positions = self.data.xpos[self.obstacle_body_ids]
        distance = np.linalg.norm(obstacle_positions - ee_pos, axis=1)
    ...
    def _compute_reward(
        ...
        # proximity penalty: if end-effector is within proximity_threshold of any obstacle,
        # apply a penalty proportional to (threshold - distance) scaled by proximity_penalty_scale.
        proximity_penalty = 0.0
        min_dist = self._min_ee_obstacle_distance()
        ...
        # store last proximity penalty for external inspection
        self._last_proximity_penalty = proximity_penalty
        self._last_proximity_distance = min_dist
        return reward
    ...
    def reset(
        ...
        *,
        ...
        mujoco.mj_resetData(self.model, self.data) # joint pos
        self._step_count = 0
    ...
    def step(
        ...
        info: Dict[str, np.ndarray] = {
            ...
            "proximity_distance": np.array([getattr(self, "_last_proximity_distance", np.nan)], dtype=np.float64),
            "proximity_penalty": np.array([getattr(self, "_last_proximity_penalty", 0.0)], dtype=np.float64),
        }
